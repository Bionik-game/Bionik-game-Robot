...
int battery_status(){
float voltage[2];
voltage[0]=(analogRead(A0))*(5.0/1023.0);// First Li-pol's cell voltage 
voltage[1]=(analogRead(A1))*(5.0/1023.0);// As above
if((voltage[0]/5.0 < 0.85) || (voltage[1]/5.0 < 0.85))
return BATT_DISCHARGED;
else
return BATT_OK;
}
...
void loop() {
  if(battery_status()==BATT_DISCHARGED)
 robot_stop();
